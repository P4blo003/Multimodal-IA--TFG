...
# Archivo: server/source/config/context.py
# Autor: Pablo González García
# Descripción:
# Módulo que se encarga de cargar la configuración del proyecto desde el 
# archivo `server_config.yaml` y variables de entorno `.env`. Valida
...
class ConfigManager:
    ...
    """
    # -- Métodos por defecto -- #
    def __init__(self, config_path:Path = Path("config/server_config.yaml"), dotenv_path:Path = Path(".env")):
        """
        ...
        self.__env['OLLAMA_HOST'] = f"{self.Config.ollama.api.host}:{self.Config.ollama.api.port}"
