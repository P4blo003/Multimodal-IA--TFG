...
# Archivo: application/common/logger.py
...
# ---- FUNCIONES ---- #
def create_logger(logger_name:str, console:bool=True, file:str=None) -> Logger:
    """
    ...
    logger.setLevel(getattr(logging, CFG.logger.level, logging.INFO))
    
    # Añade los handlers del logger.
    __formatter:Formatter = Formatter(fmt=CFG.logger.format, datefmt=CFG.logger.datefmt)     # Crea el formateador.
    
    # Si se quiere un logger de consola.
    if console:
        # Crea el handler de consola.
        __consoleHandler:StreamHandler = StreamHandler()
        __consoleHandler.setLevel(getattr(logging, CFG.logger.level, logging.INFO))
    ...
    if file:
        ...
        __logDir:str = os.path.join(CFG.system.logsDirectory, file)     # Crea la ruta del fichero.
        os.makedirs(os.path.dirname(__logDir), exist_ok=True)           # Crea el fichero.
        # Crea el handler de fichero.
        __fileHandler:RotatingFileHandler = RotatingFileHandler(__logDir, maxBytes=CFG.logger.maxBytes, backupCount=CFG.logger.backupCount, encoding='utf-8')
        __fileHandler.setLevel(getattr(logging, CFG.logger.level, logging.INFO))
